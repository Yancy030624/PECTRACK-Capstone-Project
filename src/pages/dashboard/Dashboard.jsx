...
export function Dashboard({ user, onLogout, onUserUpdated }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f7f2] text-slate-800">
      <header className="flex items-center justify-between border-b border-green-100 bg-white px-4 py-3 shadow-sm sm:px-7"><Link to="/" aria-label="Go to home"><Brand compact /></Link><div className="flex items-center gap-3"><span className="hidden text-right text-xs text-slate-500 sm:block"><strong className="block text-slate-800">{user.name}</strong>{user.role}</span><div className="grid h-9 w-9 place-items-center rounded-full bg-green-100 font-bold text-green-800">{user.name[0]}</div><button type="button" onClick={onLogout} className="rounded-lg border border-green-700 px-3 py-2 text-xs font-bold text-green-800 hover:bg-green-50">Log out</button></div></header>
