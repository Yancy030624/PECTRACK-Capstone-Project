// ============================================================================
// PECTRACK API — routes/deliveries.js (annotated for learning)
...
//
// This is the first router in the app that admits DELIVERY_PERSONNEL at
// all — every earlier router (orders, inventory, payments) deliberately
// excludes that role, each with a comment saying the real access model
// doesn't exist yet. This file IS that model: Decision 9's rule that a
// driver's own id is resolved from the session and used to scope every
// read and write (Pattern H) runs through almost every route below.
// ============================================================================
...
// Mirrors the delivery_status enum exactly, the same reasoning
// orders.js's validStatuses uses for order_status.
...
// reason and the reason it was retried are both worth keeping, and this
// phase deliberately has no delivery_status_history table to recover
// either from — so the stored column needs its own ceiling on top of the
// per-message one, or a long-lived delivery could grow it without bound.
...
// deliveries; an ADMIN gets two recovery-only transitions. Keyed by the
// TARGET status because a conditional UPDATE needs to know which source
// states it is allowed to claim from — and expressed as data rather than
// branches so the whole rule set can be read at once.
...
// A ceiling on how many proof files ONE delivery can hold. The 5mb limit
// above caps each individual request and nothing capped the total, so an
// authenticated driver could push 5mb at a time at their own delivery
// indefinitely — a slow disk-fill with a valid session behind it.
const maxProofsPerDelivery = 5

// A single row shape shared by the staff LIST, the driver's OWN list,
// PATCH /:id/assign, and PATCH /:id/status — one mapper for every route
// that returns a delivery, the same reasoning mapOrderSummary /
// mapAddressRow use elsewhere in this app.
...
// Everything a screen needs about a delivery in ONE query: the delivery
// row itself, who it's assigned to, the order it belongs to (status and
// total — so the screen never has to make a second round trip to
// routes/orders.js just to show what's being delivered), the customer's
// name, the full delivery address, and how many proof files exist for it.
// LEFT JOINs throughout because delivery_personnel_id starts NULL (nobody
// assigned yet) and address fields can be absent for the same reason
// order-detail LEFT JOINs customers for a walk-in order.
...
// All three roles this phase concerns get through the door — narrower
// per-route requireRole calls below (and in-handler role checks on
// PATCH /:id/status, which needs finer control than a route-level guard
// can express) handle where their permissions actually diverge.
...
router.get('/', requireRole('ADMIN', 'CASHIER'), async (request, response) => {
  ...
  // A CASE-based ORDER BY rather than the delivery_id insertion order —
  // this puts the deliveries that most need attention (PENDING_ASSIGNMENT,
  // then ASSIGNED, then OUT_FOR_DELIVERY) at the top of a staff screen,
  // with the two terminal states trailing at the bottom.
...
// server-side from the session, never trusted from a query param. Only
// NON-terminal deliveries — a driver's working list is what still needs
// doing, not a growing history of everything they've ever delivered.
...
// claim idiom used everywhere else in this codebase (Decision 5): the
// source state (PENDING_ASSIGNMENT) sits in the WHERE clause, and
// rowCount === 0 is what turns "someone already claimed this" into a
// clean 409 instead of silently reassigning it out from under the first
// assignment.
...
// one route, dispatched by the TARGET status rather than by a role check
// up front: an ADMIN may retry a FAILED delivery back to
...
// proof_type travels as a query parameter instead. The middleware chain
// below runs in order: requireRole FIRST (so an unauthorized caller never
// even has their body read), THEN express.raw (so proofType/ownership
// checks inside the handler only ever run against real image bytes).
router.post(
  '/:id/proof',
...
router.get('/:id/proof/:proofId', async (request, response) => {
  ...
  // but the bytes are gone (a restored backup, a hand-cleaned uploads/, a
  // future move to object storage that missed one). Without the callback,
  // sendFile's ENOENT reaches app.js's handler as a generic 500, which
  // tells the user "the service is broken" when the honest answer is
  // "that file isn't there any more" — the same distinction parseId draws
  // for a malformed id and the entity.too.large case draws for an
  // oversized upload.
  //
  // headersSent is checked because sendFile may already have begun
