...
// The hard prerequisite for delivery orders: a DELIVERY order needs an
// address_id, and nothing in this app has ever written one until now.
// Mounted at /api/addresses in app.js.
//
// Deliberately its own router rather than living in routes/customers.js:
// that router is ADMIN/CASHIER only (the staff-facing customer RECORD
// screen), and a customer managing their own addresses can't reach it at
// all. This one admits CUSTOMER (their own addresses only) plus
// ADMIN/CASHIER (acting for a customer taking a phone order) — the same
// mixed-role shape routes/payments.js already uses.
...
// both full creation (partial: false) and a partial PATCH.
...
router.post('/', async (request, response) => {
  ...
  try {
    ...
    // — see PATCH /:id for why the partial index cannot catch that state.
    // It has to gate the whole expression, not just the value written
    // below: the clearing UPDATE that follows runs off this same flag, so
    // computing them separately would let an inactive address clear the
    // customer's real default and then decline to replace it, leaving
    // them with none.
    ...
    try {
      ...
      // isActive here, and a field that is validated but then silently
      // discarded is worse than one that was never accepted. Note the
      // interaction with makeDefault above — an inactive address must
      // never be created as the default, for the same partial-index
      // reason PATCH /:id spells out; the AND below is what stops the
      // auto-default of a customer's first address from producing exactly
      // that state.
...
router.patch('/:id', async (request, response) => {
  ...
  // customer_addresses_one_default_per_customer is a PARTIAL index —
  // `WHERE is_default AND is_active` — so a row that is default but
  // INACTIVE is invisible to it and violates nothing. Without this check,
  // marking a deactivated address as the default cleared the customer's
  // real default (the UPDATE below does that unconditionally) and parked
  // the flag somewhere the index can't see, leaving the customer with NO
  // active default at all and a stale one waiting to spring back the
  // moment that address is reactivated. This is the one case where the
  // friendly precheck IS the enforcement, so it has to be right.
